...
def main(regenerate=False):
    ...
    # Figure (a): schematic of the veer-skewed wake, after Abkar et al.
    # Energies (2018) fig. 3. The wake cross-section is modeled as a unit
    # circle sheared linearly in y by delta(z) = -alpha_v * x * (z - z_h) / D,
    # i.e. (y - delta(z))^2 + (z - z_h)^2 = 1. This shear maps the circle to
    # an ellipse whose principal axes are the eigenvectors of the quadratic
    # form's matrix M below (semi-axis length = 1/sqrt(eigenvalue)).
    ax = axs[0]
    s = 0.8  # alpha_v * x / D: skew parameter for the schematic

    theta = np.linspace(0, 2 * np.pi, 200)
    ax.plot(np.cos(theta), np.sin(theta), color="k", ls="--", lw=1)
    ...
    for a, v, label in [(a_max, v_max, r"$a_\mathrm{max}$"), (a_min, v_min, r"$a_\mathrm{min}$")]:
        ...
        ax.text(v[0] * a * 1.3, v[1] * a * 1.3, label, ha="center", va="center", color="tab:blue")
